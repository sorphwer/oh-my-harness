...
if [ -z "${SPEC_PATH:-}" ]; then
  for candidate in \
    "${CODEX_HOME:-$HOME/.codex}/skills/use-json-render-cli/references/compact-ticket-spec.template.json" \
    "./npm/skills/use-json-render-cli/references/compact-ticket-spec.template.json" \
    "./skills/use-json-render-cli/references/compact-ticket-spec.template.json"
  do
    if [ -f "$candidate" ]; then
